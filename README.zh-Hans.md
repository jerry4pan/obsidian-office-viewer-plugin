...
- 可复制**幻灯片嵌入**，在 Markdown 阅读视图中显示来源驱动的单页内容；Live Preview 保留可编辑语法，不做行内渲染。
- 被引用页已删除或源演示文稿缺失时会明确提示，不会静默退回旧页码。
- 在打开的 PPTX 中按 `Cmd+F` 或 `Ctrl+F`，可搜索当前演示文稿自己编写的标题、正文、文本框、形状文字和表格单元格；用 `Enter` / `Shift+Enter` 前后循环跳转。
- 搜索查询、源幻灯片文字、片段和结果仅存在于当前视图会话，不会持久化。图片、讲者备注、母版/版式文字、图表和 SmartArt 尚未搜索。
...
- 编辑、保存、动画、解析旧版 `.ppt`、OCR、讲者备注搜索、全库搜索、主画面搜索高亮、多页或整份演示文稿嵌入、Live Preview 行内渲染、遥测、账号、授权与云服务均不在范围内。
