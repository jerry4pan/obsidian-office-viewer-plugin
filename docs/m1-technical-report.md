...
| The main seams cover success, degradation, error, and external fallback | Installed E2E covers success/error/fallback availability; the installed compatibility suite covers the two approved degraded Aiden fixtures; session tests cover recoverable navigation degradation and fallback invocation | Met |

## Verification record

All results below were produced on the implementation branch on 2026-07-14.

| Command | Result |
| --- | --- |
| `npm run verify` | PASS: 25 test files; 175 passed, 1 skipped |
...
| `npm run test:performance` | PASS: first-readable p95 33.5 ms; slide-switch p95 2.5 ms; no recorded failures |
| `npm run test:performance:baseline` | PASS: 8 tests passed, 1 skipped |

The M1 production bundle is 1,146,002 bytes. Because M1 changed that value,
