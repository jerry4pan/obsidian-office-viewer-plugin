...
The design was approved on 2026-07-14. It covers the complete M2 milestone and
does not claim M3 compatibility diagnostics, release packaging, or public
submission work.
...
- a virtualized, progressively rendered thumbnail rail;
- fit-to-window, manual zoom, and reset-to-fit controls;
- Arrow, Page Up, and Page Down navigation plus full-screen reading;
- independent current page, zoom, thumbnail scroll, and full-screen state per
  workspace leaf;
...
Extend the candidate-neutral renderer session with thumbnail, prefetch, zoom,
...
- `PptxViewerController` owns current page, zoom mode, navigation transitions,
...
- `renderThumbnail(index, container, signal)` returning a disposable resource
...
- `setZoomPercent(percent)` where `100` means fit-to-window and the percentage
  is applied relative to the current fitted scale.
...
runtime zoom, adaptive resize, and `destroy`. The rejected candidate must still
...
The ready view has a toolbar above a two-column reading body. The left column
is a roughly 168 CSS-pixel thumbnail rail and the right column is the main
slide viewport. The rail can collapse so narrow split panes remain useful.

The toolbar contains, in logical tab order:

1. Previous, current/total page, and Next;
2. one-based page input and Go;
3. Zoom out, current zoom label, Zoom in, and Fit;
4. Full screen;
5. Open in default application when available.

The default zoom state is `fit` at `100%`. The first zoom-in or zoom-out action
enters view-local manual mode. Manual values are clamped to 25–400% in 25-point
steps. Pane resize recomputes the underlying fitted scale but preserves the
manual multiplier. Fit resets to `100%` and resumes fit behavior.
...
- Zoom failure: retain the previous zoom and show a local action status.
...
errors retain focusable retry and external-open actions. Full-screen mode does
not hide the controls required to exit or navigate.
...
- reading-position tests prove default enablement, fingerprint validation,
  ...
  and last-event-wins behavior;
- session DOM tests prove thumbnail selection, zoom mode, resize semantics,
...
- zoom, fit reset, pane resize, and full-screen enter/exit;
- two leaves maintain independent page and zoom state;
