...
> Turn local PPTX presentations into searchable, source-preserving knowledge in
> Obsidian.

**Simplified Chinese**

> 把本地 PPTX 变成可搜索、可定位、可引用的 Obsidian 知识材料。

Use `PPTX`, not the broader phrase `Office files`, when describing the current
...
- Be calm, concrete, and honest about product boundaries.
- Prefer `local`, `read-only`, and `source-preserving` over vague claims such as
  `secure`, `native`, or `high fidelity`.
- Explain degradation and failure without blame.
- Avoid inflated language, competitive posturing, and promises that the
